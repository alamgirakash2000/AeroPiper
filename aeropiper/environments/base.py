...
import aeropiper
import aeropiper.macros as macros
import aeropiper.utils.sim_utils as SU
from aeropiper.renderers.base import load_renderer_config
from aeropiper.renderers.viewer import OpenCVViewer
from aeropiper.utils import SimulationError, XMLError
from aeropiper.utils.binding_utils import MjRenderContextOffscreen, MjSim

REGISTERED_ENVS = {}

# Lazily-built index to resolve short relative asset paths (e.g. "assets/foo.obj", "frame/bar.stl")
# that appear in some robot XMLs when we later load the *combined* model from an XML string (which
# otherwise loses the original base path context).
_ASSET_SUFFIX2_MAP = None


def _get_asset_suffix2_map():
    global _ASSET_SUFFIX2_MAP
    if _ASSET_SUFFIX2_MAP is not None:
        return _ASSET_SUFFIX2_MAP

    import aeropiper.models as _models

    assets_root = _models.assets_root
    mapping = {}
    ambiguous = set()

    for p in Path(assets_root).rglob("*"):
        if not p.is_file():
            continue
        rel_parts = p.relative_to(assets_root).parts
        if len(rel_parts) < 2:
            continue
        suffix2 = "/".join(rel_parts[-2:])
        if suffix2 in mapping and mapping[suffix2] != str(p):
            ambiguous.add(suffix2)
        else:
            mapping[suffix2] = str(p)

    for k in ambiguous:
        mapping.pop(k, None)

    _ASSET_SUFFIX2_MAP = mapping
    return _ASSET_SUFFIX2_MAP


def _resolve_asset_file_path(file_path: str) -> str:
    ...
    # Common case: paths already relative to assets_root (e.g. textures/foo.png, objects/bar.xml)
    p1 = os.path.join(_models.assets_root, file_path)
    if os.path.exists(p1):
        return p1

    # Some robot XMLs use a short relative convention like "assets/<file>" or "frame/<file>".
    # Resolve via a suffix index under assets_root.
    m = _get_asset_suffix2_map()
    return m.get(file_path, file_path)
...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def initialize_renderer(self):
        ...
        if self.renderer == "mujoco":
            ...
            from aeropiper.renderers.viewer import MjviewerRenderer
    ...
    def _reset_internal(self):
        ...
        if self.has_renderer and self.viewer is None:
            if self.renderer == "mujoco":
                if OpenCVViewer is None:
                    raise ImportError(
                        "renderer='mujoco' requires OpenCV (`cv2`). Install with `pip install opencv-python` "
                        "or use renderer='mjviewer'."
                    )
    ...
    def edit_model_xml(self, xml_str):
        ...
        for elem in all_elements:
            ...
            if old_path is None:
                continue
            elem.set("file", _resolve_asset_file_path(old_path))
