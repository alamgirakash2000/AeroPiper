...
import aeropiper
from aeropiper.controllers.composite.composite_controller import REGISTERED_COMPOSITE_CONTROLLERS_DICT
from aeropiper.controllers.parts.controller_factory import load_part_controller_config
from aeropiper.utils.log_utils import ROBOSUITE_DEFAULT_LOGGER


def _robosuite_repo_root() -> pathlib.Path:
    """
    Returns the repo root for this robosuite codebase.

    In this repo, we use a small `robosuite/` shim package, so `aeropiper.__file__` points inside
    that shim directory. The actual source tree (controllers/, models/, ...) lives at the repo root,
    which we append to `aeropiper.__path__` as the last entry.
    """
    return pathlib.Path(list(aeropiper.__path__)[-1])
...
def refactor_composite_controller_config(controller_config, robot_type, arms):
    ...
    config_dir = _robosuite_repo_root() / "controllers/config/robots/"
...
def load_composite_controller_config(controller: Optional[str] = None, robot: Optional[str] = None) -> Optional[Dict]:
    ...
    if controller is None:
        ...
        controller_fpath = (
            _robosuite_repo_root() / f"controllers/config/robots/default_{robot_name}.json"
        )
        if not os.path.exists(controller_fpath):
            controller_fpath = (
                _robosuite_repo_root() / "controllers/config/default/composite/basic.json"
            )
        ...
        if controller.endswith(".json"):
            ...
            controller_fpath = (
                _robosuite_repo_root() / f"controllers/config/default/composite/{controller_name}.json"
            )
    else:
...
def _get_robot_name(robot: str) -> str:
    ...
    elif "GR1ArmsOnly" in robot:
        return "gr1_arms_only"
    elif "GR1" in robot:
        return "gr1"
    elif "G1" in robot:
        return "g1"
