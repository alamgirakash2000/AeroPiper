...
FORMAT_STR = {"file": "[aeropiper %(levelname)s - %(asctime)s] ", "console": "[aeropiper %(levelname)s] "}
...
class DefaultLogger:
    """Default logger class in the aeropiper codebase."""

    def __init__(self, logger_name="aeropiper_logs", console_logging_level="INFO", file_logging_level=None):
        """
        Args:
        ...
        if file_logging_level is not None:
            ...
            log_file_path = "/tmp/aeropiper_{}_{}.log".format(time_str, os.getpid())
            fh = logging.FileHandler(log_file_path)
            print(colored("[aeropiper]: Saving logs to {}".format(log_file_path), "yellow"))
