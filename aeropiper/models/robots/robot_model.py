...
from aeropiper.models.base import MujocoXMLModel
from aeropiper.models.bases import LegBaseModel, MobileBaseModel, MountModel, NullBaseModel, RobotBaseModel
from aeropiper.utils.mjcf_utils import ROBOT_COLLISION_COLOR, array_to_string, find_elements, find_parent, new_element
from aeropiper.utils.transform_utils import euler2mat, mat2quat
...
class RobotModelMeta(type):
    ...
    def __new__(meta, name, bases, class_dict):
        ...
        # Keep base classes and non-target robots from registering into suite.ALL_ROBOTS
        _unregistered_envs = [
            "RobotModel",
            "ManipulatorModel",
            "LeggedManipulatorModel",
            # GR1 variants (we only want GR1ArmsOnly available)
            "GR1",
            "GR1FixedLowerBody",
            "GR1FloatingBody",
        ]
...
class RobotModel(MujocoXMLModel, metaclass=RobotModelMeta):
    ...
    def add_base(self, base: RobotBaseModel):
        """
        Mounts a base to the robot. Bases are defined in aeropiper.models.bases
        """
